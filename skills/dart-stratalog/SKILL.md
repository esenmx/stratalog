...
- `stratalog_dio` bodies: `redactBodyKeys` masks values `***` at any depth (defaults cover passwords, `cvv`/`cvc`, `pin`, `otp`, tokens, secrets; matching ignores case and `_`/`-`) and is **always on**, unlike header masking. `redactBodyPaths` drops a whole body — request, response and error — for endpoints sensitive as such (`{'/checkout/pay'}`); prefer it over key lists there, since it still covers the next field added to that payload. Card PAN/CVV under PCI-DSS is the canonical case.
