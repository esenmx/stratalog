...
Bodies are redacted here, not at the sink, because `trace` is **not** release-gated: `configureLogging` filters by level only when handed an explicit `minLevel`, so these lines reach the release console writer and land in logcat / os_log.

```dart
LoggerDioInterceptor(
  // Masked as `***` at any depth, in every direction. Defaults cover
  // passwords, cvv/cvc, pin, otp, tokens and secrets; matching ignores case
  // and `_`/`-`. Always on — a bearer token is worth showing on a local
  // console, a password never is.
  redactBodyKeys: {...LoggerDioInterceptor.defaultRedactBodyKeys, 'holderName'},
