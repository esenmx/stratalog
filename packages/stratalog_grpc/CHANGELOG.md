...
- `maskSensitiveValues` now defaults to `true` across all modes (matching `stratalog_dio`), keeping sensitive metadata values masked out-of-the-box unless opted out with `maskSensitiveValues: false`.
