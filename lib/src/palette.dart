...
abstract final class LogPalette {
  ...
  static ConsoleColor? levelColor(ChirpLogLevel level) {
    if (level > ChirpLogLevel.error) return critical;
    if (level >= ChirpLogLevel.error) return error;
    if (level >= ChirpLogLevel.warning) return warning;
    if (level == ChirpLogLevel.success) return success;
    return null;
  }
